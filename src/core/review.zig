...
//
// This is the half of the tool that makes notes worth collecting. A reference
...
// notes may contain newlines because they are *written*, and what goes through
// `send-keys` is the sentence that says where the file is.
//
// Pure: notes in, markdown out. No filesystem here - `io/fs.zig` writes it -
...
/// Renders the open notes, grouped by file and ordered by line within each.
///
...
/// changes.
///
/// Open and stale notes, not sent ones. A sent note has been acted on or
...
pub fn render(out: *std.ArrayList(u8), gpa: Allocator, store: *const notes.Store, n: u32) Allocator.Error!u32 {
    ...
    var written: u32 = 0;
    // Files in first-appearance order, without allocating a set: the note
    ...
    for (store.items(), 0..) |note, i| {
        ...
        var first = true;
        while (true) {
            var best: ?*const notes.Note = null;
            for (store.items()) |*m| {
            ...
            try out.appendSlice(gpa, "**");
            // A stale note says so in the file as well as on screen. The agent
...
/// The body as a markdown blockquote, so a note containing a list or a code
...
test "the review groups by file and orders by line" {
    var store: notes.Store = .init(testing.allocator);
    defer store.deinit();
...
test "sent notes stay out, so nothing is asked for twice" {
    var store: notes.Store = .init(testing.allocator);
    defer store.deinit();
...
test "a stale note says so in the file, not just on screen" {
...
test "a multi-line note stays inside its bullet" {
    ...
    defer out.deinit(testing.allocator);
    _ = try render(&out, testing.allocator, &store, 1);
    // Every line of the body is quoted, so a list inside a note does not
...
test "an empty review says so rather than being a bare heading" {
    ...
    try testing.expect(std.mem.indexOf(u8, out.items, "No open notes") != null);
}
