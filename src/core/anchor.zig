...
/// A note's position, carried across versions. `stale` once every path misses.
pub const Anchor = struct {
    line: u32,
    window_hash: u64,
    stale: bool = false,

    pub fn init(ids: []const u32, line: u32) Anchor {
        return .{ .line = line, .window_hash = windowHash(ids, line) };
    }

    /// Primary path: one table lookup, no searching (PERFORMANCE.md 3.1).
    /// Returns false when the line has no counterpart, which is where the
    /// fallback tiers will take over.
    pub fn migrate(self: *Anchor, map: LineMap) bool {
        if (self.stale) return false;
        if (map.get(self.line)) |n| {
            self.line = n;
            return true;
        }
        return false;
...
test "anchor migrates through the map" {
    var in: Interner = .{};
    defer in.deinit(testing.allocator);
    const a = try in.internLines(testing.allocator, "one\ntwo\nthree\n");
    defer testing.allocator.free(a);
    ...
    var anchor: Anchor = .init(a, 1);
    try testing.expect(anchor.migrate(map));
    try testing.expectEqual(@as(u32, 2), anchor.line);
}
