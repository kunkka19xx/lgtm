...
- [x] 5c, part: **the empty screen says what the tool is.** A clean tree is the resting state rather than an edge case - the pane sits on it for every moment the agent is thinking, and for all of the time before it has written anything - and it was one dim sentence in the top-left corner. It is now the README's wordmark centred over the version, the author and the key that opens the help, and nothing else: a screen this frequent earns its space by being quiet, so no border, no box, no tip of the day. Three decisions in it. The wordmark is a `Glyphs` field beside the box corners rather than a theme slot, because a terminal that cannot draw block elements needs a different picture and not a different colour - `ui.icons = "ascii"` gets a 26-column figlet, and the purity test that already guarded the ascii set had been **skipping every field that is not `[]const u8`**, so it was extended to walk rows of strings rather than trusting the new literal. The key is read out of the bindings rather than written as `?`, since a remapped keymap has to document itself and this is the one screen a reader who has not found the popup yet can learn it from. And the version is now a build option fed from `build.zig.zon`, which removed the second copy of the string that was sitting in `main.zig` drifting: `--version` and the splash cannot disagree because there is one string. Geometry is pure (`splash.place`), so the sizes that break it are tested without a terminal: too narrow for the picture or too short for the lines under it falls back to exactly the sentence that was there before, which is what a pane being dragged through every width on its way to 80 needs. The name is a clickable OSC 8 hyperlink to the author's GitHub rather than a printed URL, which would be noise on a screen this small; libvaxis carries it on `Segment.link` and the escape occupies no columns, so it cannot move a layout, and a terminal that will not follow it draws the plain name that would have been there anyway. `Frame.putLink` rather than a reach into `win` from the splash, because a path in the status line is the obvious next caller. Driven at 80x24, 40x24, 34x24 and 80x10, ascii and unicode, with the `?` popup floating over it, and the OSC 8 confirmed on the wire by capturing the raw tty rather than the rendered pane - a pane capture shows the name either way, which is exactly the bug that would have been missed
- [x] 5c, part: **the `?` list is tabbed, because forty-eight rows in one column is a list nobody reads to the end of.** In review mode it listed every binding at once and said `+11 more` in a 40-row pane; at 80x24, which is the pane this is designed for, most of the keymap was below the fold. Five tabs - `move`, `jump`, `send`, `find`, `view` - cycled with `H`/`L`. **The keys cost nothing**, which is what made this cheap: sideways was bound to one column of the grid, and the grid has been one column wide since the two-column layout was rejected for pushing the box past most panes, so `H` and `L` were bound to a movement that could not happen and the footer advertised them anyway. They now do something, and `Tab` keeps cycling the list the way it cycles a completion menu everywhere else. **The strip is let into the top border**, not given a row: a box already has a top edge, and a popup in an 80x24 pane cannot spend one of its rows on labels for its rows. It reuses the mechanism the footer already had - draw the border, repaint the marked spans in the accent - so a tab and a key are coloured by one rule rather than two. **The filter cuts across every tab and blanks the highlight while it has text in it**: browsing and finding are different tasks, and requiring a reader to know which tab a key was filed under before they can search for it would be the tabs charging rent. Five and no more, because the strip has to fit the border at 80 columns. Two pieces of dead weight came out with it: `Help.moveClamped` and `Help.layout`, both of which existed only to serve the column movement that no longer happens - the file list keeps its own copy, because its `H`/`L` really are a page of a grid
- [x] 5c, part: **a path too long for the file list loses its middle rather than its name.** A terminal clips from the right for nothing, and for a path that throws away the one part that identifies it: eight rows reading `apps/macos/LauncherApp/look-app/Views/Launcher/LauncherVi` are the same row eight times, since the directories they share are all that survived and the names that told them apart are what fell off. `ui/path.zig` is a pure function over bytes and a width method - the name is kept whole and the head is spent, so `apps/macos/LauncherApp/loo…LauncherView.swift` still says roughly where in the tree the reader is. When even the name will not fit, the *name* is elided in its middle rather than at one end, because names that collide collide at one end: `LauncherView.swift` and `LauncherViewModel.swift` differ only in the middle. The counts were being pushed off the row by a long path and now keep their column, so a narrow pane shows the name *and* the size of the change instead of neither. `wrap.zig` grew `fitFront` and `fitBack` rather than the elision walking graphemes itself - the stepping and its ASCII fast path already lived there, and a second copy would disagree the first time one met a double-width glyph. The ellipsis is a `Glyphs` field, so `ui.icons = "ascii"` gets `...` and the purity test covers it. **Two regressions from the tabs came out with it, both found by driving a 60-column pane rather than by a test.** The footer had grown five columns and was being dropped *whole* when it did not fit, which at 60 columns left an empty bottom border - the only thing on screen that says what the keys are. `footerOf` now sheds whole trailing groups instead, because half of `<Esc> close` says nothing. And `H`/`L` were sharing one word across two overlays that no longer do the same thing: it is a tab in `?` and a page of the grid in the file list, so the binding is split by mode and each says its own. That split tripped `every binding is live in at least one mode`, which had been checking three of the four mode flags and would have rejected the first finder-only binding anyone wrote
...
1. ~~Review notes~~ **DONE, ahead of the phase order.** They are called *comments* now - `comment` is what every review tool calls the thing you write on a line - so `core/comments.zig` (jsonl store in `.lgtm/comments.jsonl`, read once from `notes.jsonl` and rewritten, owned bytes from the session allocator, open/sent/stale), `<Space>c` / `<Space>lc` / `<Space>dc` / `<Space>vc` / `<Space>sc` / `]c [c`, gutter markers in three colours, `core/review.zig` renders `review-N.md`, `Ctrl-s` writes it and sends exactly one line. Two things went differently from the sketch. Re-anchoring is per-file `anchor.carryLine` on every re-diff rather than a batched pass - the store snapshots the working text of note-carrying files before the arena resets, because that is the only moment both versions exist. And **stale notes go into the review file**, marked, rather than being held back: hard rule 7 does not stop at the screen, and a remark the reader has not dismissed is still theirs to send. `Ctrl-e` was not built; the compose box is the editor, and it is modal
...
3. Native filesystem watching behind the same `watch.zig` interface
   ...
   Also done out of order, and small enough to note here rather than number: a file over `large_file_lines` renders as a summary row and `zo` opens it (`zc` folds it again). `core/diff.zig` had `materialise` and its tests from phase 2; nothing called it, so a big file was a dead end. `ui/review.zig` remembers what was opened and materialises it again before the buffers are attached, or the next re-diff folds it back under the reader
...
7. Config surface. **Mostly done:** `[keys]` remaps any command by the spelling the `?` popup prints - including the compose box's own keys, which are `Modes.compose_only` bindings rather than keys the box knows; `[presets]` fills its `Ctrl-i` list; `[review] ignore` keeps generated files out of the review; `[ui] comments` and `[ui] compose` place them; `[nav] mark_on_submit` decides whether `<C-s>` takes the mark. Every message that names a key reads it from the keymap, so a remap cannot leave the screen advertising a key nobody has. What is left: the keymap *presets* (vim/helix/emacs/plain) and user `[templates]`
8. Turn checkpoints. **Done in memory, ahead of the phase order** (`core/checkpoint.zig`): `m` marks, `M` and `:nomark` drop it, `]m` / `[m` walk what arrived since, `<C-s>` takes the mark as part of submitting, and a bar in the gutter says which rows are new. It is the same line map anchoring already runs, pointed at a different pair of texts - 0.93 ms per re-diff over fourteen changed files. Two departures from the sketch: it **annotates rather than filters**, because a new line is unjudgeable without the old ones around it, so `a` was never needed; and it compares working trees rather than diffs, which is what keeps it a lookup. What is left is item 2 - the mark is a field on `Review` and dies with the process, where SNAPSHOTS.md §5.1 wants a ref name in `.lgtm/state.json` and the same state the pending badge reads (NOTIFICATIONS.md §4 rule 3)
