...
- Source: `git diff` against HEAD (staged + unstaged). Agent-agnostic by construction. In a repository whose first commit has not happened, `HEAD` does not resolve and the source is `git diff --cached` plus the untracked scan, so every file reads as new - which it is. In a directory that is not a repository at all there is no source: the review is empty and the empty screen says so, because a tool that reads `git diff` for a living should say what is missing rather than crash on it. It says three things - what is wrong, **which directory it means**, and `cd to a repository, or git init here`. The path is the useful half: standing in the wrong directory is a likelier reason to be reading that screen than having meant to review an uninitialised one, and only the path tells those apart. The hint offers both readings and presumes neither, because "run `git init`" told to someone who mistyped a `cd` leaves a repository behind in their downloads folder.
